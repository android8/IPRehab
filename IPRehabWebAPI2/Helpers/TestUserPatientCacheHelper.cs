...
namespace IPRehabWebAPI2.Helpers
{
  ...
  public class TestUserPatientCacheHelper : ITestUserPatientCacheHelper
  {
    ...
    public TestUserPatientCacheHelper(MasterreportsContext context)
    {
      _context = context;
    }
    ...
    /// <returns></returns>
    public async Task<List<PatientDTO>> GetPatients(IFSODPatientRepository _patientRepository, string criteria, string orderBy, int pageNumber, int PageSize, string patientID)
    {
            ...
            case "none":
              {
                ...
                if (rawPatients.Any())
                {
                  ...
                  var viewablePatients = rawPatients.Where(p => p.Facility.Contains("648"));
