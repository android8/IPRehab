...
import { Utility, UserAnswer, AjaxPostbackModel, EnumChangeEventArg, EnumDbCommandType } from "./commonImport.js";
...
const formController = (function () {
    ...
    /* private function */
    function submitTheForm(saveBtn: any, dialogOptions: any): void {
        ...
        function updateDomAttributes(apiBaseUrl: string, questionController: string, stage: string, episodeIdAfterFristPost: any): void {

            const thisUrl: string = apiBaseUrl + '/api/' + questionController + '/GetStageAsync/' + stage + '?includeAnswer=true&episodeID=' + episodeIdAfterFristPost;
            $.ajax({
                'url': thisUrl,
                'type': 'GET',
                'dataType': 'json',
        ...
        //use jquery ajax
        function jQueryAjax(apiBaseUrl: string, apiCotroller: string, stage: string, postBackModel: AjaxPostbackModel, episodeID: number) {
            ...
            $.ajax({
                ...
                .done(function (result) {
                    ...
                    let dialogText: string = 'Data is saved (Done).';
                    ...
                    reloadPageAfterPost();
                    const questionController: string = 'question';
        ...
        $('.spinnerContainer').show();
        const deleteAnswers: Array<UserAnswer> = new Array<UserAnswer>();
        const insertAnswers: Array<UserAnswer> = new Array<UserAnswer>();
        ...
        persistables.each(function () {
            ...
            const oldAnswer: string = thisPersistable.data('oldvalue');

            const currentAnswer: string = commonUtility.getControlCurrentValue(thisPersistable);
            const CRUD: EnumDbCommandType = commonUtility.getCRUD(thisPersistable, oldAnswer, currentAnswer);
            ...
            const controlType: string = thisPersistable.prop('type');
            ...
            const AnswerByUserID: string = thisPersistable.data('userid');
            ...
            if (/GG0130/i.test(thisPersistableID) || /GG0170/i.test(thisPersistableID))   //regex
            {
                ...
                if (thisAnswerScoreElement.length > 0) {
                    let thisAnswerScore: number = +commonUtility.getControlScore(thisAnswerScoreElement);    //parseInt(thisAnswerScoreElement.data('score'));
    ...
    /* private function */
    function addMoreDate(stage: string) {
    ...
    function changeCssClassIfOldAndNewValuesAreDifferent() {
        ...
        $('input.persistable, select.persistable').not(Q12).each(function () {
            ...
            thisPersistable.on('change', function () {
                const controlType: string = thisPersistable.prop('type');
                const controlId: string = thisPersistable.prop('id');
                const thisControlLabel: any = $('#' + controlId + '_label');
                const Q12_or_Q23_is_empty: boolean = commonUtility.isEmpty(Q12) || commonUtility.isEmpty(Q23);
                const currentAnswer: any = commonUtility.getControlCurrentValue(thisPersistable);
                const oldAnswer: any = thisPersistable.data('oldvalue');
                const changeType: EnumDbCommandType = commonUtility.getCRUD(thisPersistable, oldAnswer, currentAnswer);
                const admitDate: Date = new Date(commonUtility.getControlCurrentValue(Q12));
                const onsetDate: Date = new Date(commonUtility.getControlCurrentValue(Q23));
                const minDate = new Date('2020-01-01 00:00:00');
                const is_onset_on_or_later_than_admit_and_valid: boolean = onsetDate > minDate && admitDate > minDate && admitDate >= onsetDate;
                const noMoreChangeCssClass = $('.persistable.changedFlag, .persistable.Create, .persistable.Update, .persistable.Delete').length === 0;
                const saveButton: any = $('#saveButton');
    ...
    function Score_GG0170RandS_Performance(performanceType: string = ''): number {
        ...
        let GG0170I_Score: number = 0, GG0170R_Score: number = 0, GG0170S_Score: number = 0;
