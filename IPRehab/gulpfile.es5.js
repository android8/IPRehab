...
gulp.task('minifyAppJs', function () {
    return gulp.src(script.myJs[0], { allowEmpty: true }).pipe(minify({
        noSource: false,
        ext: {
            min: '.min.js'
        }
...
gulp.task('copyAppJsMap', function () {
    return gulp.src(script.myMap[0]).pipe(gulp.dest(script.output[0]));
});

gulp.task('minifyAppModelJs', function () {
    return gulp.src(script.myJs[1], { allowEmpty: true }).pipe(minify({
        noSource: false,
        ext: {
            min: '.min.js'
        }
...
gulp.task('copyMyAppModel', function () {
    return gulp.src(script.myJs[1]).pipe(gulp.dest(script.output[0]));
});

gulp.task('copyAppModelJsMap', function () {
    return gulp.src(script.myMap[1]).pipe(gulp.dest(script.output[0]));
});
...
gulp.task('compileMySassStyles', function (done) {
    gulp.src(style.mySass).pipe(sass() /*compile sass */
    .on('error', sass.logError)).pipe(gulp.dest(style.output));
    done();
});

gulp.task('minifyMyCss', function () {
    return gulp.src(style.myCssOnly, { allowEmpty: true }).pipe(minify({
        noSource: false,
        ext: {
            min: '.min.css'
        }
